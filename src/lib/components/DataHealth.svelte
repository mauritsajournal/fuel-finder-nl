<script lang="ts">
	...
	function freshnessColor(isoDate: string): string {
		...
		if (hours > 8) return 'text-amber-400';
...
{#if metadata && fuelUpdated}
	<div class="pointer-events-none absolute bottom-16 left-0 z-10 p-3 sm:bottom-20">
		...
		<button
			onclick={toggle}
			class="glass pointer-events-auto rounded-xl px-2.5 py-1.5 text-[11px] transition-all duration-200 hover:bg-white/10 {freshnessColor(fuelUpdated)}"
		...
		{#if expanded}
			<div class="glass pointer-events-auto mt-2 w-56 rounded-xl p-3">
				<h3 class="mb-2 text-xs font-semibold text-white">Data status</h3>

				<div class="space-y-1.5 text-[11px]">
					<!-- Fuel data -->
					<div class="flex items-center justify-between">
						<span class="text-slate-400">Brandstof</span>
					...
					<!-- EV data -->
					<div class="flex items-center justify-between">
						<span class="text-slate-400">EV laders</span>
						{#if evUpdated}
							<span class={freshnessColor(evUpdated)}>
								{relativeTime(evUpdated)}
							</span>
						{:else}
							<span class="text-slate-600">niet beschikbaar</span>
						{/if}
					</div>

					<!-- POI data -->
					<div class="flex items-center justify-between">
						<span class="text-slate-400">Voorzieningen</span>
						{#if poiUpdated}
					...
					<div class="mt-2 border-t border-white/5 pt-2">
						<div class="flex items-center justify-between">
							<span class="text-slate-400">Stations geladen</span>
							<span class="text-white">{stations.length}</span>
						</div>
						{#if metadata.chargerCount > 0}
							<div class="flex items-center justify-between">
								<span class="text-slate-400">Laadpunten</span>
								<span class="text-white">{metadata.chargerCount}</span>
							</div>
						{/if}
					</div>
				</div>
